...
type CursorState = "default" | "hover" | "view" | "explore" | "cta";

declare global {
  interface Window {
    __cursorState?: CursorState;
    __setCursor?: (s: CursorState) => void;
  }
}

export function setCursorState(s: CursorState) {
  if (typeof window !== "undefined") window.__setCursor?.(s);
}
...
export default function CustomCursor() {
  ...
  useEffect(() => {
    ...
    window.__setCursor = (s: CursorState) => setState(s);
  ...
  // Auto-detect interactive hover -> hover state
  useEffect(() => {
    if (!enabled) return;
    ...
    <>
      ...
      <motion.div
        ...
        animate={{
          width: size,
          height: size,
          backgroundColor: isLabel ? "rgba(255,255,255,0.96)" : "rgba(255,255,255,0)",
          borderColor: isLabel ? "rgba(255,255,255,1)" : state === "default" ? "rgba(255,255,255,0.55)" : "#891FFB",
      ...
      >
        ...
        />
        <span
          className="relative font-display text-[10px] font-bold tracking-[0.12em] text-black"
