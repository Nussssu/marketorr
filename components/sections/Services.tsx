...
export default function Services() {
  ...
  return (
    <section id="services" className="relative bg-[#08080A] py-24 md:py-36">
      <div className="container-x">
        <SectionLabel index="02" name="SERVICES" />
        <div className="mt-10 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <RevealText as="h2" className="display-lg uppercase text-white" lines={["Branding", "& UI/UX"]} />
          <motion.p
            initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }} transition={{ duration: 0.7 }}
            className="max-w-md text-[15px] leading-relaxed text-[#9A9AA3]"
          >
        ...
        <div className="mt-14 border-t border-white/10">
          {SERVICES.map((s) => {
            ...
            const grad = s.accentTo
              ...
              : s.accent;
            return (
              <div key={s.slug} className="group relative border-b border-white/10">
                <Link
                ...
                >
                  ...
                  <span
                    ...
                    aria-hidden
                  />
                  <span className="font-display text-sm font-bold text-white/35">{s.index}</span>
                  <span>
                    <span className="flex items-center gap-4">
                      <span className="font-display text-3xl font-extrabold uppercase tracking-tight text-white transition-colors duration-300 md:text-5xl">
                        {s.name}
                      </span>
                      <span
                    ...
                    <span className="mt-2 block max-w-xl text-[14px] text-[#9A9AA3]">{s.short}</span>
                    <AnimatePresence initial={false}>
                      {(isHover || isOpen) && (
                        ...
                        >
                          <span className="mt-4 block text-[14px] leading-relaxed text-white/70">{s.description}</span>
                          ...
                          <span className="mt-5 hidden items-end gap-2 md:flex" aria-hidden>
                            ...
                            <span className="ml-3 font-display text-[11px] font-bold tracking-[0.2em] text-white/50">
                  ...
                  <span className="flex items-center gap-4">
                    <span className="hidden text-[12px] font-bold uppercase tracking-[0.18em] text-white/40 group-hover:text-white md:block">
                      {isOpen ? "Open" : "Explore"}
                    </span>
                    <span
                      className="btn-press flex h-12 w-12 items-center justify-center rounded-full border border-white/15 text-lg transition-all duration-500 group-hover:border-transparent group-hover:text-white"
                ...
                {/* mobile tap expand */}
                <button
                  className="btn-press mb-4 ml-1 rounded-full border border-white/15 px-4 py-2 text-[11px] font-bold uppercase tracking-[0.16em] text-white/70 md:hidden"
