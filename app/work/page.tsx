...
export const metadata = { title: "Our Work — Marketorr" };
  ...
  return (
    <div className="bg-[#08080A] pb-24 pt-32">
      <div className="container-x">
        ...
        <h1 className="display-lg mt-8 uppercase text-white">Work that <span className="text-gradient">creates impact.</span></h1>
        <div className="mt-12 grid gap-10 md:grid-cols-2">
          {PROJECTS.map((p) => (
            <Link key={p.slug} href={`/work/${p.slug}`} data-cursor="view" className="group btn-press block overflow-hidden rounded-2xl border border-white/10 bg-[#111116]">
              ...
              <div className="p-6">
                ...
                <h2 className="mt-1 font-display text-2xl font-extrabold uppercase text-white">{p.title}</h2>
                <div className="mt-3 flex gap-2">{p.tags.map((t) => <Tag key={t} accent={p.accent}>{t}</Tag>)}</div>
              </div>
            </Link>
          ))}
        </div>
